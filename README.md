...
**中文 AI 生成文本的句法标记：一项基于 283 万字平行语料的对照研究**

**摘要** 本研究考察大语言模型生成的中文文本与人类写作在句法与篇章层面的系统性差异。以 629 篇文章（2,826,972 汉字，95,551 句，45,721 段）构建对照语料，其中 300 篇由五个主流模型在受控条件下生成，329 篇为人类作者的公开发表文本。研究检验了 26 项在公共讨论中被广泛认作"AI 文风"的候选特征，逐项计算生成侧与人类侧的频率比。结果显示，其中 15 项不具备区分力，部分项的方向与流行认知相反：人类使用比喻的频率为生成文本的 2.4 倍，正文设问句为 17 倍，句内同构排比亦高于生成文本。11 项通过检验的特征集中于篇章衔接与结构层面，而非词汇或标点层面，其中区分力最强者为段首零回指评论（4.4 倍）。研究同时报告了六次因算子设计缺陷导致的测量失误及其修正过程，并讨论了单一模型样本、分母选择与语料不可核验三项方法局限。
...
《经济学人》2026 年 7 月发表的同类研究（55,940 句，1,200,000 词，对照 ChatGPT / Claude / Gemini / Grok）报告生成文本标点密度偏低。本研究在中文语料中仅部分重复了该结论。

| 指标 | 生成 | 人类 | 与英文研究 |
|---|---|---|---|
| 逗号 | 39.39 | 56.78 | 一致 |
| 短句率（≤15 字） | 14.9% | 18.4% | 一致 |
| 抽象后缀词（性/化/度/主义） | 8.61 | 5.28 | 一致 |
| 分号 | 1.64 | 0.95 | 相反 |
| 括号 | 3.50 | 2.10 | 相反 |
| 引号 | 14.65 | 4.87 | 相反 |

引号一项差异最大且方向相反。英文研究将标点偏少部分归因于生成文本不引用信息源，但中文语料中生成侧引号密度为人类侧的三倍，其成因是生成虚构对话而非引述实际来源。"不引用"这一倾向在中文语料中以另一形式呈现：数字密度为人类侧的 36%，时间锚点为 39%。此项构成本研究第 8 条规则的依据。

该研究另有两点方法论提示与本研究直接相关：单一词形不足以作为判据（原文类比为，因某文本使用 delve 即判定为生成文本，等同于因使用 imprudence 即判定作者为奥斯汀）；所有频率结论均有时效性，模型迭代持续向人类文风收敛。
...
| 提示性冒号 无差异 | 6431 条命中中，标题 1266、列表 1225、对话 207，均属规则明示豁免者 | *R* = 4.2 |
